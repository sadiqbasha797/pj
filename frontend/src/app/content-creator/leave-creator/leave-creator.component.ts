...
@Component({
  selector: 'app-leave-creator',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './leave-creator.component.html'
})
export class LeaveCreatorComponent implements OnInit {
  holidays: Holiday[] = [];
  newHoliday = {
    startDate: '',
    endDate: '',
  ...
  loadHolidays() {
    this.creatorService.fetchHolidays().subscribe({
  ...
  applyForHoliday() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.creatorService.applyForHoliday(this.newHoliday).subscribe({
          next: () => {
            Swal.fire({
              icon: 'success',
              title: 'Success',
  ...
  withdrawHoliday(holidayId: string) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.creatorService.withdrawHoliday(holidayId).subscribe({
          next: () => {
            Swal.fire({
              icon: 'success',
              title: 'Success',
